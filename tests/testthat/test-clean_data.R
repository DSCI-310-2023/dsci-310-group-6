...
test_that("`clean_data` should return a data frame", {
  expect_is(clean_data(file_path, output_path = NULL, c("instant", "dteday", "yr", "holiday", "casual", "registered")), "data.frame")
})
...
test_that("`clean_data` returns a dataframe with 2 columns", {
  expect_equal(ncol(clean_data(file_path, output_path = NULL,c("instant", "dteday", "yr", "holiday", "casual", "registered"))), 10)
})
...
are passed to `file_path` and `cols` arguments", {
  expect_error(count_classes(hi, output_path = NULL, c("instant", "dteday", "yr", "holiday", "casual", "registered")))
