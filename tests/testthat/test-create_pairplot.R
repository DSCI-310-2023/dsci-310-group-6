...
test_input <- here("tests/testthat/iris-dummy.csv")
...
test_that("`create_pairplot` should have correct title 'Flower Pairplot'", {
  ...
  test_plot <- create_pairplot(test_input, NULL, plot_title = expected_title)
...
are passed to `results` arguments", {
  expect_error(create_pairplot(test_input, NULL, hi))
